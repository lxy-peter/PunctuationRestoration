...
import shutil
import subprocess
from utils.os import list_visible, reset
...
class KaldiFeaturizer(BaseFeaturizer):
    ...
    def __init__(self, config):
        """
        :param config: Portion of configuration dictionary under `"data"`-->`"audio"`-->`"featurizer"`. This should
        contain a `"kaldi_eg"` key, whose corresponding value is the path to the Kaldi example to use.
        """
    ...
    def __call__(self, split_path):
        ...
        dataset.
        :param split_path: Path to a dataset's split, which should conform to standard data format (see README file).
        """
    ...
    def _align(self, output_path):
        ...
        :param output_path: Path to `feat/[split]/kaldi` directory, where the Kaldi outputs are saved.
        :return: A tuple `(phones, words)`. `phones` is a dictionary of aligned phones and words. `words` is a
        ...
        with open(os.path.join(output_path, 'text'), 'r') as f:
            lines = f.read().split('\n')
        try:
            while True:
                lines.remove('')
        ...
        with open(os.path.join(self.kaldi_eg, 'data/lang_rescore/phones/align_lexicon.int'), 'r') as f:
    ...
    def _create_data(self, src):
        ...
        :param src: Path to source dataset, which should conform to standard data format (see README file).
        :param kaldi_eg: Path to a Kaldi example directory, e.g. kaldi/egs/tedlium/s5_r3/. This directory should contain
        a `data/` subdirectory.
        :return: None.
        """
    ...
    def _create_wavscp(self, audio_path, save):
        ...
        :param audio_path: Path to `audio/` directory, which should conform to standard data format (see README file).
        :param save: Directory in which to save the created `wav.scp` file.
        :return: None
        """
    ...
    def _create_text(self, text_path, save):
        ...
        :param text_path: Path to `text/` directory, which should conform to standard data format (see README file).
        :param save: Directory in which to save the created `text` file.
        :return: None
        """
        ...
        with open(os.path.join(save, 'text'), 'w') as text:
            for filename in txts:
                with open(os.path.join(text_path, filename), 'r') as f:
                    single_text = f.read().strip().lower()
    ...
    def _read_words(self, path):
        ...
        :param path: Path to file whose lines should be of the format `<word> <int>`.
        :return: Dictionary with words as keys and integers as values.
        """
    ...
    def _split_scp(self, dir):
        ...
        :param dir: Path to directory containing .scp files.
        :return: None.
        """
        ...
        for scp in scps:
            ...
            with open(os.path.join(self.split_dir, scp), 'r') as f:
                lines = f.read().split('\n')
            try:
                while True:
                    lines.remove('')
    ...
    def read_kaldi(self, filename, filetype):
        ...
        :param filetype: Type of file, e.g. `"ark"` or `"scp"`.
