...
class MultimodalFeaturizer(BaseFeaturizer):
    ...
    def __init__(self, config):
        ...
        :param config: Configuration dictionary under `"data"`. This should contain both `"audio"` and`"text"` keys.
        """
    ...
    def concat_emb(self, audio_featurizer, text_featurizer):
        ...
        combined = ''
        for scp in scps:
            with open(os.path.join(self.output_path, scp), 'r') as f:
                combined += f.read()
    ...
    def concat_one_ark(self, audio_featurizer, text_featurizer, kaldi_dict):
        ...
        for k in kaldi_dict.keys():
            ...
            with open(os.path.join(self.split_path, 'text', k + '.txt'), 'r') as f:
                text = f.read().strip()
    ...
    def make_examples(self):
        ...
        for file in egs_txt_files:
            with open(os.path.join(self.output_path, 'egs_txt', file), 'r') as f:
                lines = f.read().split('\n')
            try:
                while True:
                    lines.remove('')
...
SUPPORTED_FEATURIZERS = {
    ("Kaldi", "BERTMLP"): MultimodalFeaturizer,
}
